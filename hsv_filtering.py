...
def calculate_clustering_accuracy(predicted_labels, true_labels):
    """
    Calculate clustering accuracy considering possible label permutations.

    Args:
        predicted_labels (numpy.ndarray): Labels from clustering (0 or 1)
        true_labels (numpy.ndarray): Ground truth labels from YOLO file (1 or 2)

    Returns:
        float: Best accuracy score
    ...
    if accuracy1 >= accuracy2:
        ...
        final_predictions = 1 - predicted_labels

    # Calculate confusion matrix
    confusion_matrix = {
        'true_positive': np.sum((final_predictions == 1) & (true_labels == 1)),
        'true_negative': np.sum((final_predictions == 0) & (true_labels == 0)),
        'false_positive': np.sum((final_predictions == 1) & (true_labels == 0)),
        'false_negative': np.sum((final_predictions == 0) & (true_labels == 1))
    }
